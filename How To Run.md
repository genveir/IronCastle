...
Categories are dotted paths. `IronCastle` is the parent category and holds five leaf categories:

- **IronCastle.Oracles.** Every oracle table, one document per table. Read-only in practice.
- **IronCastle.State.** `game-state.md`, `character-bron.md` and `places.md`.
...
- `replace_document_section`, `append_to_document` and `delete_document_section` change one part of a document and leave the rest alone. Prefer these for any partial change.
...
- **places.md**: locations established at the table. The campaign document covers the castle's overall layout; places.md records what has been seen and fixed in play.
...
4. Call `document_index` on `IronCastle.Promises`, `IronCastle.Oracles`, `IronCastle.Townies`, `IronCastle.Holidays` and `IronCastle.Sessions` once, and keep the lists.
5. Fetch promise documents, Townie documents and `places.md` when they come up in play, not in advance.
...
1. Call `request_write_permission` for each leaf you will write to. This is usually `IronCastle.Sessions`, `IronCastle.State` and `IronCastle.Promises`, plus `IronCastle.Townies` if a Townie was made or changed.
...
8. Update `places.md`: append a section for each new location, and replace the section of any that changed.
